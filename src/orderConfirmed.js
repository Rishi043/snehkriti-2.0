...
if (!order) {
  ...
  const itemsHtml = order.items.map(i => `
    <div class="flex justify-between items-center py-2 border-b border-[#d4a373] border-opacity-20">
      <div>
        <span style="${DM}font-size:0.9rem;color:#2d1f14;">${i.name}</span>
        ${i.size ? `<span style="${DM}font-size:0.72rem;background:#feeafa;color:#d4a373;padding:2px 8px;border-radius:20px;margin-left:6px;">${i.size}</span>` : ''}
        <span style="${DM}font-size:0.8rem;color:#999;margin-left:4px;">×${i.qty}</span>
      </div>
      <span style="${DM}font-size:0.9rem;font-weight:700;color:#d4a373;">₹${i.price * i.qty}</span>
    </div>`).join('');
  ...
  root.innerHTML = `
    ...
    <div class="grid md:grid-cols-2 gap-8 mb-10">
      <div class="summary-card">
        <h3 class="text-xl font-bold text-[#d4a373] mb-4" style="font-family:'DM Sans',sans-serif;">What you ordered &#x1F9FE;</h3>
        ${itemsHtml}
        <div class="mt-4 space-y-1">
          <div class="flex justify-between text-sm" style="font-family:'DM Sans',sans-serif;"><span>Subtotal</span><span>&#8377;${order.subtotal}</span></div>
          <div class="flex justify-between text-sm" style="font-family:'DM Sans',sans-serif;">
            <span>Delivery</span>
            <span>${order.delivery === 0 ? '<span class="text-green-500 font-semibold">FREE &#x1F389;</span>' : '&#8377;' + order.delivery}</span>
          </div>
          <div class="flex justify-between font-bold text-[#d4a373] border-t border-[#d4a373] border-opacity-30 pt-2" style="font-family:'DM Sans',sans-serif;font-size:1.1rem;">
            <span>Total</span><span>&#8377;${order.total}</span>
          </div>
        </div>
        <div class="mt-4 pt-4 border-t border-[#d4a373] border-opacity-20">
          <p class="text-sm" style="font-family:'DM Sans',sans-serif;color:#5e503f;">
            &#128205; ${order.customer.address1}<br>
            ${order.customer.city}, ${order.customer.state} &mdash; ${order.customer.pincode}
          </p>
        </div>
      </div>

      <div class="watercolor-bg rounded-2xl p-6 text-center">
        <h3 class="text-xl font-bold text-[#d4a373] mb-4" style="font-family:'DM Sans',sans-serif;">Complete Your Payment &#x1F4B3;</h3>
        <div class="bg-white rounded-xl p-4 inline-block shadow-lg mb-4">
          <img src="/images/payment-qr.png" alt="Payment QR Code"
            ...
            onerror="this.parentElement.innerHTML='<div style=\'width:192px;height:192px;display:flex;align-items:center;justify-content:center;background:#f8edeb;border-radius:12px;\'><p style=\'font-family:DM Sans,sans-serif;color:#d4a373;text-align:center;font-size:0.85rem;\'>QR Code<br>Coming Soon</p></div>'">
        </div>
        <p class="text-lg font-bold text-[#d4a373] mb-1" style="font-family:'DM Sans',sans-serif;">Scan to pay &#8377;${order.total} via UPI</p>
        <p class="text-sm text-[#6c757d] italic mb-4" style="font-family:'DM Sans',sans-serif;">After paying, send your screenshot on WhatsApp or Instagram DM</p>
    ...
    <div class="flex flex-wrap justify-center gap-4">
      <a href="products.html" class="submit-btn text-lg px-8 py-3 rounded-full" style="font-family:'DM Sans',sans-serif;">Continue Shopping &#x1F6CD;&#xFE0F;</a>
    </div>`;
}
