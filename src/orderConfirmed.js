  ...
  root.innerHTML = `
    ...
    <div class="grid md:grid-cols-2 gap-8 mb-10">
      <div class="summary-card">
        <h3 class="text-xl font-bold text-[#d4a373] mb-4" style="${DM}">What you ordered 🧾</h3>
        ${itemsHtml}
        <div class="mt-4 space-y-1">
          <div class="flex justify-between text-sm" style="${DM}"><span>Subtotal</span><span>₹${order.subtotal}</span></div>
          <div class="flex justify-between text-sm" style="${DM}">
            <span>Delivery</span>
            <span>${order.delivery === 0 ? '<span class="text-green-500 font-semibold">FREE 🎉</span>' : '₹' + order.delivery}</span>
          </div>
          <div class="flex justify-between font-bold text-[#d4a373] border-t border-[#d4a373] border-opacity-30 pt-2" style="${DM}font-size:1.1rem;">
            <span>Total</span><span>₹${order.total}</span>
          </div>
        </div>
        <div class="mt-4 pt-4 border-t border-[#d4a373] border-opacity-20">
          <p class="text-sm" style="${DM}color:#5e503f;">
      ...
      <div class="watercolor-bg rounded-2xl p-6 text-center">
        <h3 class="text-xl font-bold text-[#d4a373] mb-4" style="${DM}">Complete Your Payment 💳</h3>
        ...
        <p class="text-lg font-bold text-[#d4a373] mb-1" style="${DM}">Scan to pay ₹${order.total} via UPI</p>
        <p class="text-sm text-[#6c757d] italic mb-4" style="${DM}">After paying, send screenshot on WhatsApp or Instagram DM</p>
    ...
    <div class="flex flex-wrap justify-center gap-4">
      <a href="products.html" class="submit-btn text-lg px-8 py-3 rounded-full" style="${DM}">Continue Shopping 🛍️</a>
    </div>`;
}
