...
if (!order) {
  ...
  const itemsHtml = order.items.map(i => `
    <div class="flex justify-between items-center py-2 border-b border-[#d4a373] border-opacity-20">
      <div>
        <span class="handwritten text-lg text-[#5e503f]">${i.name}</span>
        ${i.size ? `<span class="bg-[#feeafa] text-[#d4a373] px-2 py-0.5 rounded-full text-xs handwritten ml-2">${i.size}</span>` : ''}
        <span class="handwritten text-sm text-[#6c757d] ml-2">×${i.qty}</span>
      </div>
  ...
  root.innerHTML = `
    <div class="text-center mb-12">
      ...
      <p class="handwritten text-xl text-[#5e503f] mb-4">Thank you <strong class="text-[#d4a373]">${order.customer.name}</strong>! Your piece of art is on its way to being made with love.</p>
      <div class="price-tag bg-[#feeafa] text-[#d4a373] px-6 py-3 rounded-full text-xl font-bold shadow-md inline-block">
        Order ID: ${order.orderId}
      </div>
    </div>

    <div class="grid md:grid-cols-2 gap-8 mb-10">
      <div class="summary-card artistic-border">
        <h3 class="handwritten text-2xl font-bold text-[#d4a373] mb-4">What you ordered 🧾</h3>
        ${itemsHtml}
        <div class="mt-4 space-y-1">
          <div class="flex justify-between handwritten text-lg"><span>Subtotal</span><span>₹${order.subtotal}</span></div>
          <div class="flex justify-between handwritten text-lg">
            <span>Delivery</span>
            <span>${order.delivery === 0 ? '<span class="text-green-500">FREE 🎉</span>' : '₹' + order.delivery}</span>
          </div>
        ...
        <div class="mt-4 pt-4 border-t border-[#d4a373] border-opacity-20">
          <p class="handwritten text-base text-[#5e503f]">
      ...
      <div class="watercolor-bg artistic-border rounded-2xl p-6 text-center">
        ...
        <p class="handwritten text-xl font-bold text-[#d4a373] mb-2">Scan to pay ₹${order.total} via UPI</p>
        <p class="handwritten text-base text-[#6c757d] italic">📸 After paying, screenshot your payment and send it to our WhatsApp or Instagram DM</p>
        <a href="https://wa.me/${OWNER_PHONE}?text=${payText}"
          target="_blank" class="whatsapp-btn mt-4 inline-flex">
    ...
    <div class="flex flex-wrap justify-center gap-4">
      <button onclick="ownerWA()" class="submit-btn handwritten text-lg px-6 py-3 rounded-full">Send My Order to Snehkriti on WhatsApp 💬</button>
      <button onclick="customerWA()" class="submit-btn-outline handwritten text-lg px-6 py-3 rounded-full">Get My Order Confirmation on WhatsApp ✨</button>
      <a href="products.html" class="handwritten text-xl text-[#d4a373] hover:underline self-center">Continue Shopping 🛍️</a>
    </div>`;

  window.ownerWA = () => sendOwnerWhatsApp(order);
  window.customerWA = () => sendCustomerWhatsApp(order);
}
