...
if (!product) {
    ...
    return `
      <div>
        ...
        <a href="${product.instagramUrl}" target="_blank"
          class="handwritten"
          style="margin-top:12px;display:inline-flex;align-items:center;gap:8px;padding:8px 18px;border-radius:24px;color:white;font-size:1rem;text-decoration:none;background:linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);">
          📸 View on Instagram
        </a>
      </div>`;
  }
  ...
  function buildProductInfo() {
    ...
    const sizeOptions = availableSizes.map(s => {
      ...
      return `
        <div style="position:relative;">
          ...
          <label for="size-${s}" class="handwritten" style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:8px 18px;background:#fff;border-radius:24px;border:1.5px solid #e2d0c0;cursor:pointer;font-size:1.1rem;font-weight:600;color:#5e503f;min-width:52px;transition:all 0.2s;gap:2px;">
            ${s}
    ...
    return `
      <div>
        <span class="handwritten" style="display:inline-block;background:#d4a373;color:white;font-size:0.9rem;padding:3px 14px;border-radius:20px;margin-bottom:10px;">${product.badge}</span>

        <h2 class="handwritten" style="font-size:1.8rem;font-weight:700;color:#d4a373;margin-bottom:8px;line-height:1.3;">${product.name}</h2>

        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
          <span id="price-display" class="handwritten" style="font-size:1.6rem;font-weight:700;color:#d4a373;">₹${basePrice}</span>
          <span id="size-surcharge-note" class="handwritten" style="font-size:0.9rem;color:#d4a373;display:none;">+₹${LARGE_SIZE_SURCHARGE} for XL</span>
        </div>

        <p style="font-size:0.9rem;color:#5e503f;line-height:1.65;margin-bottom:14px;">${product.description}</p>

        ${product.note ? `<p class="handwritten" style="font-size:0.85rem;color:#e91e63;background:#feeafa;padding:4px 12px;border-radius:20px;display:inline-block;margin-bottom:12px;">⚠️ ${product.note}</p>` : ''}

        <div style="background:#fdf8f5;border-radius:12px;padding:14px;border:1px solid #e8d5c0;margin-bottom:14px;">
          <p class="handwritten" style="font-size:1rem;font-weight:700;color:#5e503f;margin-bottom:4px;">Select Size</p>
          <p style="font-size:0.75rem;color:#999;margin-bottom:10px;">XL is +₹${LARGE_SIZE_SURCHARGE} extra</p>
          <div style="display:flex;flex-wrap:wrap;gap:8px;">${sizeOptions}</div>
          <p id="size-error" class="handwritten" style="font-size:0.85rem;color:#e91e63;margin-top:8px;display:none;">Please select a size 🌸</p>
        </div>

        <div style="display:flex;align-items:center;gap:12px;background:#fdf8f5;border-radius:12px;padding:12px 14px;border:1px solid #e8d5c0;margin-bottom:16px;">
          <span class="handwritten" style="font-size:1rem;font-weight:700;color:#5e503f;">Qty</span>
          <div style="display:flex;align-items:center;gap:10px;">
            <button class="qty-btn" onclick="changeQty(-1)">−</button>
            <span id="qty-display" class="handwritten" style="font-size:1.2rem;font-weight:700;color:#d4a373;min-width:24px;text-align:center;">1</span>
            <button class="qty-btn" onclick="changeQty(1)">+</button>
          </div>
          <span id="line-total" class="handwritten" style="font-size:0.9rem;color:#999;"></span>
        </div>

        <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px;">
          <button id="add-cart-btn" onclick="handleAddToCart()" class="submit-btn handwritten" style="font-size:1.1rem;">Add to Cart 🛍️</button>
          <button onclick="handleBuyNow()" class="submit-btn-outline handwritten" style="font-size:1.1rem;">Buy Now ✨</button>
        </div>

        <a href="https://wa.me/${OWNER_PHONE}?text=${waText}" target="_blank" class="whatsapp-btn handwritten" style="margin-bottom:16px;display:inline-flex;font-size:1rem;">
  ...
  function buildRelated() {
    ...
    const cards = related.map(p => `
      <div class="pcard" style="flex:0 0 180px;min-width:180px;scroll-snap-align:start;">
        ...
        <div class="pcard-body">
          ...
          <a href="product-detail.html?id=${p.id}" class="btn-view" style="display:inline-block;">View</a>
        </div>
      </div>`).join('');
    return `
  ...
  content.innerHTML = `
    <div style="margin-bottom:16px;">
      <a href="products.html" class="handwritten" style="font-size:1rem;color:#d4a373;text-decoration:none;">← Back to Collections</a>
    </div>
